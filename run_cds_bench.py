...
class Test_Params:
    ...
    test_dirs = ['tests/benchmarks/benchmarks_bounds']

    N = 1      # no. of runs per tests
    T = [1,1,1,1,1,1,1,1]  # values to be passed to -t flags. should be in increasing order
    Depth = [inf,inf,inf,inf,inf,inf,inf,inf]
    Fence = [inf,inf,inf,inf,1,2,3,4]
    Cycle = [1,2,3,4,inf,inf,inf,inf]
    Parallel = False
...
class Test_Stats:
    def __init__(self, filepath, t=inf, d=inf, f=inf,c=inf) -> None:
        ...
        self.execute_test(filepath, t, d, f, c)
    ...
    def execute_test(self, filepath, t, d, f, c):
        for i in range(Test_Params.N):
            run_once = Run_Test(filepath, t, d, f, c)
...
class Run_Test:
    ...
    def __init__(self, filepath, t, d, f, c) -> None:
        ...
        self.run_test_once(filepath, t, d, f, c)
    ...
    def run_test_once(self, filepath, t, d, f, c):
        ...
        if c != inf:
            process_command.append('-c')
            process_command.append(str(c))
...
def run_all_config(filename):
    ...
    for i in range(len(Test_Params.T)): 
        ...
        d_conf = Test_Params.Depth[i]
        f_conf = Test_Params.Fence[i]
        c_conf = Test_Params.Cycle[i]
        exec_test = Test_Stats(filename, t=t_conf,d=d_conf,f=f_conf, c=c_conf)
...
def write_csv_header(csv_file_name):
    ...
    for i in range (len(Test_Params.T)):
        ...
        csv_header_t = csv_header_t.replace(',', 
                        '(t=' + str(Test_Params.T[i]) + ' d=' + str(Test_Params.Depth[i])+ ' f=' + str(Test_Params.Fence[i])+ ' c=' + str(Test_Params.Cycle[i])+'),')
...
# tracer, seqlock test, c11tester can detect assert violation or only data races, genmc test on smallest test
# check fences manually, modify tests, 
