...
void push(mystack_t *s, unsigned int val) {
	...
	while (true) {
		// acquire
		oldTop = atomic_load_explicit(__LINE__, &s->top, acquire);
		newTop = MAKE_POINTER(nodeIdx, get_count(oldTop) + 1);
		// relaxed
		atomic_store_explicit(__LINE__, &node->next, oldTop, relaxed);
...
unsigned int pop(mystack_t *s)
{
	...
	while (true) {
		...
		// relaxed
		next = atomic_load_explicit(__LINE__, &node->next, relaxed);
