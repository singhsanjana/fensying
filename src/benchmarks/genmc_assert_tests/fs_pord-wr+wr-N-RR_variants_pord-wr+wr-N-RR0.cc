...
void *thread_n(void *arg)
{
	...
	for (int i = thr->thrid; i < NUM_BATCHES * WRITER_BATCH_SIZE; i += thr->thrcnt) {
		...
		/* Wait to be signaled */
		__VERIFIER_assume(atomic_load_explicit(__LINE__, &condition[thr->thrid], memory_order_acquire));
		...
		atomic_store_explicit(__LINE__, &condition[thr->thrid], 0, memory_order_release);
		atomic_store_explicit(__LINE__, &condition[(thr->thrid + 1) % (thr->thrcnt)], 1, memory_order_release);
	}
	return NULL;
}
...
int user_main()
{
	...
	atomic_store_explicit(__LINE__, &condition[0], 1, memory_order_relaxed);
	for (int i = 1u; i < NUM_WRITERS; i++)
		atomic_store_explicit(__LINE__, &condition[i], 0, memory_order_relaxed);
