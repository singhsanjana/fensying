...
def convert_load(line):
    ...
    elif 'atomic_load' in line:
        return convert_action(line, 'atomic_load')
...
def convert_store(line):
    ...
    elif 'atomic_store' in line:
        return convert_action(line, 'atomic_store')
...
def convert_fetch_add(line):
    ...
    elif 'atomic_fetch_add' in line:
        return convert_action(line, 'atomic_fetch_add')
...
def convert_user_main(line):
    print ('adding argc and argv')
    return line[:line.find('(')+1] + 'int argc, char **argv' + line[line.find('(')+1:]
...
def convert_files(root, files):
    ...
    for filename in files:
        ...
        for line in lines:
            ...
            elif 'atomic_load_explicit' in line or '.load' in line or 'atomic_load' in line:
                line = convert_load(line)
            elif 'atomic_store_explicit' in line or '.store' in line or 'atomic_store' in line:
                line = convert_store(line)
            elif 'atomic_fetch_add_explicit' in line or '.fetch_add' in line or 'fetch_add' in line:
            ...
            elif '__VERIFIER_pbarrier' in line:
                line = ''

            if 'user_main' in line and not 'int argc' in line:
                line = convert_user_main(line)
            newlines += line
