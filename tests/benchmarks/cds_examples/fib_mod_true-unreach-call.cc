/**
 * CDSChecker - fib_mod_true-unreach-call
*/
...
static void check_assert(void* arg) {
	int c_i = count_i.load(__LINE__, memory_order_acquire);
	int c_j = count_j.load(__LINE__, memory_order_acquire);
	if ((c_i == NUM and c_j == NUM - 1) or (c_i == NUM - 1 and c_j == NUM)) {
		int temp_i = i.load(__LINE__, memory_order_relaxed);
		int temp_j = j.load(__LINE__, memory_order_relaxed);
		MODEL_ASSERT(temp_i == FIB_NUM || temp_j == FIB_NUM);
	}
}
