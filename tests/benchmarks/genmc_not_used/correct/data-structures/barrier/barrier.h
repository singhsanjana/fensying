...
{
	unsigned int step = atomic_load(__FILE__, __LINE__, &b->step_);

	if (atomic_fetch_add(__FILE__, __LINE__, &b->nwait_, 1) == b->n_ - 1) {
		/* OK, last thread to come.  */
		atomic_store(__FILE__, __LINE__, &b->nwait_, 0); // XXX: maybe can use relaxed ordering here ??
		atomic_fetch_add(__FILE__, __LINE__, &b->step_, 1);
		return true;
	} else {
		while (atomic_load(__FILE__, __LINE__, &b->step_) == step)
			;
		return false;
	}
}
