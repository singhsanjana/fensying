...
{
	struct queue_node *node = atomic_load_explicit(__FILE__, __LINE__, &q->tail, memory_order_acquire);
...
{
	struct queue_node *head = atomic_load_explicit(__FILE__, __LINE__, &q->head, memory_order_acquire);
	struct queue_node *node = atomic_load_explicit(__FILE__, __LINE__, &head->next, memory_order_acquire);
