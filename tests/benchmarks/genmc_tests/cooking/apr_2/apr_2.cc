...
extern int pthread_kill (thrd_t __threadid, int __signo) __attribute__ ((__nothrow__ ));
...
extern int pthread_create (thrd_t *__restrict __newthread,
...
extern int pthread_join (thrd_t __th, void **__thread_return);
# 282 "/usr/include/pthread.h" 3 4
extern int pthread_detach (thrd_t __th) __attribute__ ((__nothrow__ ));



extern thrd_t pthread_self (void) __attribute__ ((__nothrow__ )) __attribute__ ((__const__));


extern int pthread_equal (thrd_t __thread1, pthread_t __thread2)
  __attribute__ ((__nothrow__ )) __attribute__ ((__const__));
...
extern int pthread_setschedparam (thrd_t __target_thread, int __policy,
      const struct sched_param *__param)
     __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (3)));


extern int pthread_getschedparam (thrd_t __target_thread,
      int *__restrict __policy,
      struct sched_param *__restrict __param)
     __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (2, 3)));


extern int pthread_setschedprio (thrd_t __target_thread, int __prio)
     __attribute__ ((__nothrow__ ));
...
extern int pthread_cancel (thrd_t __th);




extern void thrd_testcancel (void);
...
extern int pthread_getcpuclockid (thrd_t __thread_id,
      __clockid_t *__clock_id)
     __attribute__ ((__nothrow__ )) __attribute__ ((__nonnull__ (2)));
...
typedef thrd_t apr_os_thread_t;
...
static apr_status_t proc_mutex_proc_thrd_tryacquire(apr_proc_mutex_t *mutex)
{
    apr_status_t rv;
...
static const apr_proc_mutex_unix_lock_methods_t mutex_proc_pthread_methods =
{
    ...
    proc_mutex_proc_thrd_tryacquire,
                                            ...
                                            apr_pool_t *pool)
{
    ...
    (*new)->td = (thrd_t *)apr_pcalloc(pool, sizeof(pthread_t));
    ...
    if ((stat = thrd_create((*new)->td, (thrd_start_t)& dummy_worker, NULL)) == 0) {
        return 0;
    }
    else {
