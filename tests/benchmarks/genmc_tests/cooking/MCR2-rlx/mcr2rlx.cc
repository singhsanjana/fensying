...
void *thread_1(void *ptr)
{
	for (int i = 0; i < 2; i++) {
		...
		if (atomic_load_explicit(__FILE__, __LINE__, &x, memory_order_relaxed) > 0) {
...
{
	if (atomic_load_explicit(__FILE__, __LINE__, &x, memory_order_relaxed) > 1) {
		if (atomic_load_explicit(__FILE__, __LINE__, &y, memory_order_relaxed) == 3) {
			/* assert(0); */
			/* printf("*****error*****\n"); */
			;
		} else {
