...
struct kvs_data {
	...
	kvs_data(int sz) {
		...
		for (i = 2; i < real_size; i++) {
			_data[i].store(__FILE__, __LINE__, NULL, memory_order_release);
		}
		_data[1].store(__FILE__, __LINE__, hashes, memory_order_release);
	}

	~kvs_data() {
		int *hashes = (int*) _data[1].load(__FILE__, __LINE__, memory_order_acquire);
		delete hashes;
		delete[] _data;
	}
};
...
friend class CHM;
	...
	*/
	...
	class CHM {
		...
		public:
		CHM(int size) {
			_newkvs.store(__FILE__, __LINE__, NULL, memory_order_release);
			_size.store(__FILE__, __LINE__, size, memory_order_release);
			_slots.store(__FILE__, __LINE__, 0, memory_order_release);
	
			_copy_idx.store(__FILE__, __LINE__, 0, memory_order_release);
			_copy_done.store(__FILE__, __LINE__, 0, memory_order_release);
		}
		...
		bool table_full(int reprobe_cnt, int len) {
			return
				...
				_slots.load(__FILE__, __LINE__, memory_order_acquire) >= reprobe_limit(len);
		}
	
		kvs_data* resize(cliffc_hashtable *topmap, kvs_data *kvs) {
			//model_print("resizing...\n");
			/**** FIXME: miss ****/
			kvs_data *newkvs = _newkvs.load(__FILE__, __LINE__, memory_order_acquire);
			if (newkvs != NULL)
				return newkvs;
	
			// No copy in-progress, start one; Only double the table size
			int oldlen = kvs->_size;
			int sz = _size.load(__FILE__, __LINE__, memory_order_acquire);
			int newsz = sz;
			...
			/**** FIXME: miss ****/
			newkvs = _newkvs.load(__FILE__, __LINE__, memory_order_acquire);
			//model_print("hey1\n");
			...
			//model_print("hey2\n");
			newkvs->_data[0].store(__FILE__, __LINE__, chm, memory_order_release);
	
			kvs_data *cur_newkvs; 
			// Another check after the slow allocation
			/**** FIXME: miss ****/
			if ((cur_newkvs = _newkvs.load(__FILE__, __LINE__, memory_order_acquire)) != NULL)
				return cur_newkvs;
			...
			if (!_newkvs.compare_exchange_strong(__FILE__, __LINE__, desired, expected, memory_order_acq_rel,
		...
		void help_copy_impl(cliffc_hashtable *topmap, kvs_data *oldkvs,
			...
			int panic_start = -1;
			int copyidx;
			while (_copy_done.load(__FILE__, __LINE__, memory_order_acquire) < oldlen) {
				copyidx = _copy_idx.load(__FILE__, __LINE__, memory_order_acquire);
				if (panic_start == -1) { // No painc
					copyidx = _copy_idx.load(__FILE__, __LINE__, memory_order_acquire);
					while (copyidx < (oldlen << 1) &&
						!_copy_idx.compare_exchange_strong(__FILE__, __LINE__, copyidx, copyidx +
							min_copy_work, memory_order_acq_rel, memory_order_acq_rel))
						copyidx = _copy_idx.load(__FILE__, __LINE__, memory_order_acquire);
					if (!(copyidx < (oldlen << 1)))
						panic_start = copyidx;
				}
		...
		void copy_check_and_promote(cliffc_hashtable *topmap, kvs_data*
			oldkvs, int workdone) {
			int oldlen = oldkvs->_size;
			int copyDone = _copy_done.load(__FILE__, __LINE__, memory_order_acquire);
			if (workdone > 0) {
				while (true) {
					copyDone = _copy_done.load(__FILE__, __LINE__, memory_order_acquire);
					if (_copy_done.compare_exchange_weak(__FILE__, __LINE__, copyDone, copyDone +
						workdone, memory_order_acq_rel, memory_order_acq_rel))
						break;
				}
			}
			...
			if (copyDone + workdone == oldlen &&
				topmap->_kvs.load(__FILE__, __LINE__, memory_order_acquire) == oldkvs) {
				/**** FIXME: miss ****/
				kvs_data *newkvs = _newkvs.load(__FILE__, __LINE__, memory_order_acquire);
				/**** CDSChecker error ****/
				topmap->_kvs.compare_exchange_strong(__FILE__, __LINE__, oldkvs, newkvs, memory_order_acq_rel,
					memory_order_acq_rel);
			}
		}
	...
	cliffc_hashtable() {
		...
		*/
		...
		kvs->_data[0].store(__FILE__, __LINE__, chm, memory_order_release);
		_kvs.store(__FILE__, __LINE__, kvs, memory_order_release);
	}
	...
	*/
	TypeV* get(TypeK *key) {
		...
		/**** CDSChecker error ****/
		kvs_data *kvs = _kvs.load(__FILE__, __LINE__, memory_order_acquire);
		/**
			//@Begin
...
//		@Begin
	...
	*/
	...
	static CHM* get_chm(kvs_data* kvs) {
		CHM *res = (CHM*) kvs->_data[0].load(__FILE__, __LINE__, memory_order_acquire);
		return res;
	}

	static int* get_hashes(kvs_data *kvs) {
		return (int *) kvs->_data[1].load(__FILE__, __LINE__, memory_order_acquire);
	}
	...
	static inline slot* key(kvs_data *kvs, int idx) {
		...
		slot *res = (slot*) kvs->_data[idx * 2 + 2].load(__FILE__, __LINE__, memory_order_acquire);
		/**
			@Begin
	...
	*/
	...
	static inline slot* val(kvs_data *kvs, int idx) {
		...
		slot *res = (slot*) kvs->_data[idx * 2 + 3].load(__FILE__, __LINE__, memory_order_acquire);
		/**
			@Begin
			# This is a complicated potential commit point since many many functions are
			# calling val().
	...
	static inline bool CAS_key(kvs_data *kvs, int idx, void *expected, void *desired) {
		bool res = kvs->_data[2 * idx + 2].compare_exchange_strong(__FILE__, __LINE__, expected,
			desired, memory_order_acq_rel, memory_order_acq_rel);
		/**
		...
		while (true) {
			...
			*/
			...
			if (++reprobe_cnt >= REPROBE_LIMIT ||
				...
				/**** FIXME: miss ****/
				kvs_data *newkvs = chm->_newkvs.load(__FILE__, __LINE__, memory_order_acquire);
				/**
					//@Begin
					@Commit_point_define_check: true
	...
	*/
	static slot* putIfMatch(cliffc_hashtable *topmap, kvs_data *kvs, slot
		*key_slot, slot *val_slot, slot *expVal) {
			...
			if (K == NULL) { // Get a free slot
				...
				if (CAS_key(kvs, idx, NULL, key_slot)) {
					/**
						...
						@End
					*/
					chm->_slots.fetch_add(__FILE__, __LINE__, 1, memory_order_acq_rel); // Inc key-slots-used count
		...
		/**** FIXME: miss ****/
		newkvs = chm->_newkvs.load(__FILE__, __LINE__, memory_order_acquire);
		/**
			//@Begin
			@Commit_point_define_check: true
				...
				*/
				if (expVal != NULL) { // Not called by a table-copy
					...
					if ((V == NULL || V == TOMBSTONE) &&
						val_slot != TOMBSTONE)
						chm->_size.fetch_add(__FILE__, __LINE__, 1, memory_order_acq_rel);
					if (!(V == NULL || V == TOMBSTONE) &&
						val_slot == TOMBSTONE)
						chm->_size.fetch_add(__FILE__, __LINE__, -1, memory_order_acq_rel);
				}
	...
	kvs_data* help_copy(kvs_data *helper) {
		/**** FIXME: miss ****/
		kvs_data *topkvs = _kvs.load(__FILE__, __LINE__, memory_order_acquire);
		CHM *topchm = get_chm(topkvs);
		// No cpy in progress
		if (topchm->_newkvs.load(__FILE__, __LINE__, memory_order_acquire) == NULL) return helper;
