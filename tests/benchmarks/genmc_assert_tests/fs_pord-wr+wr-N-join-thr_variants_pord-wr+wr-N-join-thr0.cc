...
#define DECLARE_WRITER(id)						\
	...
	{								\
		...
		close(fd);						\
		atomic_store_explicit(__LINE__, &done[id], 1, memory_order_release); \
		return NULL;						\
	}
...
void *thread_joiner(void *unused)
{
	...
	do {
		...
		while (!atomic_load_explicit(__LINE__, &done[1], memory_order_acquire) &&
		       !atomic_load_explicit(__LINE__, &done[2], memory_order_acquire) &&
		       !atomic_load_explicit(__LINE__, &done[3], memory_order_acquire) &&
		       !atomic_load_explicit(__LINE__, &done[4], memory_order_acquire))
			;
