...
void enqueue(queue_t *q, unsigned int val)
{
	...
	tmp = atomic_load_explicit(__LINE__, &q->nodes[node].next, relaxed);
	set_ptr(&tmp, 0); // NULL
	atomic_store_explicit(__LINE__, &q->nodes[node].next, tmp, relaxed);

	while (!success) {
		tail = atomic_load_explicit(__LINE__, &q->tail, acquire);
		next = atomic_load_explicit(__LINE__, &q->nodes[get_ptr(tail)].next, acquire);
		if (tail == atomic_load_explicit(__LINE__, &q->tail, relaxed)) {
			...
			if (!success) {
				unsigned int ptr = get_ptr(atomic_load_explicit(__LINE__, &q->nodes[get_ptr(tail)].next, acquire));
...
bool dequeue(queue_t *q, unsigned int *retVal)
{
	...
	while (!success) {
		head = atomic_load_explicit(__LINE__, &q->head, acquire);
		tail = atomic_load_explicit(__LINE__, &q->tail, relaxed);
		next = atomic_load_explicit(__LINE__, &q->nodes[get_ptr(head)].next, acquire);
		if (atomic_load_explicit(__LINE__, &q->head, relaxed) == head) {
