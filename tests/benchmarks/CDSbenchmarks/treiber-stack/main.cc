...
static void main_task(void *param)
{
	...
	if (pid % 4 == 0) {
		atomic_store_explicit(__FILE__, __LINE__, &x[1], 17, relaxed);
		push(stack, 1);
	} else if (pid % 4 == 1) {
		atomic_store_explicit(__FILE__, __LINE__, &x[2], 37, relaxed);
		push(stack, 2);
	} else if (pid % 4 == 2) {/*
		idx1 = pop(stack);
		if (idx1 != 0) {
			a = atomic_load_explicit(__FILE__, __LINE__, &x[idx1], relaxed);
			printf("a: %d\n", a);
		}*/
	} else {
		idx2 = pop(stack);
		if (idx2 != 0) {
			b = atomic_load_explicit(__FILE__, __LINE__, &x[idx2], relaxed);
			printf("b: %d\n", b);
		}
	}
}
