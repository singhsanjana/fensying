...
class Entry {
	...
	Entry(int h, Key *k, Value *v, Entry *n) {
		this->hash = h;
		this->key = k;
		this->value.store(__FILE__, __LINE__, v, relaxed);
		this->next.store(__FILE__, __LINE__, n, relaxed);
	}
};
...
class Segment {
	...
	void lock() {
		...
		int from = 0;
		int to = 1;
		while (!segMutex.compare_exchange_strong(__FILE__, __LINE__, from, to, seq_cst));
	}
...
class HashMap {
	...
	Value* get(Key *key) {
		...
		while (e != NULL) {
			if (e->hash == hash && eq(key, e->key)) {
				res = e->value.load(__FILE__, __LINE__, seq_cst);
				if (res != NULL)
					return res;
				else
					break;
			...
			e = e->next.load(__FILE__, __LINE__, relaxed);
		}
		...
		Entry *newFirstPtr = first->load(__FILE__, __LINE__, relaxed);
		if (e != NULL || firstPtr != newFirstPtr) {
			e = newFirstPtr;
			while (e != NULL) {
				if (e->hash == hash && eq(key, e->key)) {
					// Protected by lock, no need to be SC
					res = e->value.load(__FILE__, __LINE__, relaxed);
	...
	Value* put(Key *key, Value *value) {
		...
		Entry *firstPtr = first->load(__FILE__, __LINE__, relaxed);
		e = firstPtr;
		while (e != NULL) {
			if (e->hash == hash && eq(key, e->key)) {
				// FIXME: This could be a relaxed (because locking synchronize
				// with the previous put())??  no need to be acquire
				oldValue = e->value.load(__FILE__, __LINE__, relaxed);
				e->value.store(__FILE__, __LINE__, value, seq_cst);
		...
		// first->store(__FILE__, __LINE__,  newEntry, relaxed);
		seg->unlock(); // Critical region ends
		return NULL;
	}
	...
	Value* remove(Key *key, Value *value) {
		...
		e->value.store(__FILE__, __LINE__, NULL, relaxed);

		// The strategy to remove the entry is to keep the entries after the
		// removed one and copy the ones before it
		Entry *head = e->next.load(__FILE__, __LINE__, relaxed);
		Entry *p;
		p = first->load(__FILE__, __LINE__, relaxed);
		while (p != e) {
			head = new Entry(p->hash, p->key, p->value.load(__FILE__, __LINE__, relaxed), head);
			p = p->next.load(__FILE__, __LINE__, relaxed);
		}

		// Publish the new head to readers 
		first->store(__FILE__, __LINE__,  head, release);
		// first->store(__FILE__, __LINE__,  head, relaxed);
		seg->unlock(); // Critical region ends
		return oldValue;
	}
};
