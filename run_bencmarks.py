...
class Test_Params:
    ...
    test_dirs = ['tests/benchmarks/store_buffer','tests/benchmarks/seqlocks','tests/benchmarks/peterson','tests/benchmarks/linuxrwlocks','tests/benchmarks/lamport','tests/benchmarks/flipper','tests/benchmarks/dekker_fen','tests/benchmarks/dekker','tests/benchmarks/burns_fen','tests/benchmarks/burns','tests/benchmarks/barrier',
        'tests/benchmarks/bakery'
    ]

    N = 5       # no. of runs per tests
    T = [inf]  # values to be passed to -t flags. should be in increasing order
    Depth = [inf]
    Parallel = False
...
def run_all_config(filename):
    ...
    for i in range(len(Test_Params.T)): 
        ...
        exec_test = Test_Stats(filename, t=t_conf,d=d_conf)
