...
/*
 * user_main() reads a file in batches and dispatches the batches to different
...
int user_main()
{
	...
	for (int i = 0u; i < FILESIZE; i += BATCH_SIZE, ++count) {
		...
		atomic_store_explicit(__LINE__, &threads[count].data, buf[count], memory_order_relaxed);
		atomic_store_explicit(__LINE__, &threads[count].ready, true, memory_order_release);
	}
